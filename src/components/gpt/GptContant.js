...
const GeminiContent = () => {
  const { gptSearchNames, gptSearchMovies, gptSearchBtnClicked } = useSelector((store) => store.gpt)
        ...
        {gptSearchBtnClicked &&
          (!(gptSearchNames == null) && gptSearchBtnClicked ? (
            gptSearchNames.map((moviesName, idx) => (
              <MoviesPlayer key={moviesName} title={moviesName} movies={gptSearchMovies[idx]} />
            ))
          ) : (
            <Shimmer />
          ))}
